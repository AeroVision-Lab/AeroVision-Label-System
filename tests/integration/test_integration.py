...
TEST_DATABASE = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "labels.db"
)
    ...
    if not existing_files:
        ...
        try:
            ...
            for i in range(5):
                ...
                draw.text(
                    ((800 - text_width) / 2, 400), text, fill=(0, 0, 0), font=font
                )
...
class TestAIIntegration:
    ...
    @pytest.mark.skipif(
        not os.path.exists(TEST_IMAGES_DIR), reason="Test images not available"
    )
    ...
    def test_reject_prediction(self, client, test_images):
        ...
        if response.status_code == 200 and response.get_json()["items"]:
            ...
            if response.status_code == 200:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT processed FROM ai_predictions WHERE filename = ?",
                    (filename,),
                )
    ...
    def test_complete_workflow(self, client, test_images):
        ...
        if pending_data["items"]:
            ...
            if approve_response.status_code == 200:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT * FROM labels WHERE id = ?", (approve_data["id"],)
                )
                label = cursor.fetchone()
                conn.close()
        ...
        if response.status_code == 200:
            ...
            if len(items) > 1:
                ...
                for i in range(len(regular_indices) - 1):
                    ...
                    conf1 = min(
                        items[idx1]["aircraft_confidence"],
                        items[idx1]["airline_confidence"],
                    )
                    conf2 = min(
                        items[idx2]["aircraft_confidence"],
