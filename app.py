        ...
        if os.path.exists(IMAGES_DIR):
            for filename in os.listdir(IMAGES_DIR):
                if (
                    ...
                    and filename not in labeled_files
                    and filename not in skipped_files
                    and filename not in predicted_files
                ):
        ...
        for pred in batch_result["predictions"]:
            if "error" not in pred:
                try:
                    ...
                    print(
                        f"[ERROR] Failed to save prediction for {pred.get('filename', 'unknown')}: {e}"
                    )

        print(
            f"[INFO] Startup AI prediction completed: {success_count}/{len(image_paths)} succeeded"
        )
...
def get_images():
            ...
            if (
                ...
                if ai_prediction:
                    image_info["ai_prediction"] = {
                        ...
                        "registration_confidence": ai_prediction[
    ...
    return jsonify({"total": len(images), "items": images})
...
def create_airline():
    ...
    try:
        ...
        return jsonify(
            {"id": airline_id, "code": data["code"], "name": data["name"]}
        ), 201
    except Exception as e:
...
def release_lock():
    ...
    return jsonify(
        {"message": "释放成功" if success else "无需释放", "filename": filename}
    )
...
def get_ai_status():
    """获取AI服务状态"""
    if not ai_enabled:
        return jsonify({"enabled": False, "message": "AI predictor not initialized"})
    ...
    try:
        ...
        return jsonify({"enabled": True, "config": config, "services": services})
    except Exception as e:
        return jsonify({"enabled": False, "message": str(e)}), 500
...
def run_ai_predict_batch():
    ...
    try:
        ...
        if not image_paths:
            return jsonify({"message": "No new images to predict", "count": 0})
...
def get_pending_reviews():
    ...
    return jsonify({"total": len(predictions), "items": predictions})
...
def bulk_approve_predictions():
    ...
    for filename in filenames:
        try:
            ...
            if not ai_pred:
                errors.append(
                    {"filename": filename, "error": "AI prediction not found"}
                )
                continue
            ...
            # 更新标注的AI相关字段
            db.update_label_with_ai_data(
                result["id"], {"review_status": "auto_approved", "ai_approved": 1}
            )

            results.append(
                {"filename": filename, "id": result["id"], "file_name": new_filename}
            )
...
def reject_prediction():
    ...
    try:
        ...
        return jsonify({"message": "Prediction rejected"})
